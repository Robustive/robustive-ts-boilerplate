...
// main.ts で trigger している bootユースケースの結果でエラーが発生していないかを監視
...
<template>
  <v-app-bar>
    ...
    <v-btn v-if="states.shared.signInStatus.case === SignInStatus.keys.signOut" append-icon="mdi-login"
      @click="onClickSignIn">
      サインイン
    </v-btn>
    <v-btn v-else-if="states.shared.signInStatus.case === SignInStatus.keys.unknown" disabled append-icon="mdi-login">
      サインイン
    </v-btn>
  ...
  <drawer v-model="state.isDrawerOpen" :signInStatus="states.shared.signInStatus"
    :items="states.application.drawerItems" @click:signIn="onClickSignIn" />
  <v-main>
    <router-view />
  </v-main>
  <v-dialog v-model="state.isDialogOpen" width="auto">
    <v-card width="400" prepend-icon="mdi-check" :title="state.dialogTitle || ''" :text="state.dialogBody || ''">
